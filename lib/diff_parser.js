module.exports = function() {
    ...
    DIFF.findTestsForEachCard = function(cardObjects, cb) {
        DIFF.findTestsForEachCardRecursively(cardObjects, 0, cb);
    };

    DIFF.findTestsForEachCardRecursively = function(cardObjects, index, cb) {
        var cmdArgs = DIFF.getDiffCommandArgs(cardObjects[index]);
        DIFF.runner.run("git", cmdArgs, function(diffResponse) {
            var testFiles = DIFF.findTests(diffResponse);
            cardObjects[index].commits = undefined;
            cardObjects[index].testsFound = [];
            for(var i=0; i<testFiles.length; i++){
                var testsObject = { file:testFiles[i].name, tests:[] };
                for(var j=0; j<testFiles[i].tests.length; j++){
                    if(testFiles[i].tests[j].wasModified === true){
                        if(testFiles[i].tests[j].name) {
                            testsObject.tests.push(testFiles[i].tests[j].name);
                        }
                    }
                }
                if(testsObject.tests.length>0) {
                    cardObjects[index].testsFound.push(testsObject);
                }
            }
            index++;
            if(index < cardObjects.length) {
                DIFF.findTestsForEachCardRecursively(cardObjects, index, cb);
            } else {
                cb && cb(cardObjects);
            }
        });
